...
import { excessRecords, plansByStatus } from '../../store/selectors';

export const ShiftsLayout = () => {
  const { plans, records, features } = useOTStore();

  const pendingCount = plansByStatus(plans, 'pending').length;
  const reconcileCount =
    plansByStatus(plans, 'reconciling').length + excessRecords(records).length;
  ...
  const tabs = [
    ...
    {
      to: '/shifts/reconciliation',
      label: 'Reconciliation',
      show: features.overtimePlanner && features.attendanceReconciliation,
      badge: reconcileCount,
    },
    ...
    <div>
      <PageHeader
        breadcrumb={
          <SmartBreadcrumb
            items={[
              { label: 'Settings', to: '/shifts/scheduler' },
              { label: 'Attendance' },
        ...
        description="Plan the week, pre-plan overtime against budget, and reconcile against attendance."
        border={false}
      />
        ...
        <div className="flex gap-5 overflow-x-auto">
          {tabs.map((t) => (
            <NavLink
              ...
              to={t.to}
              className={({ isActive }) =>
                `pb-2.5 -mb-px text-13 font-medium border-b-2 transition whitespace-nowrap inline-flex items-center gap-1.5 ${
                  isActive
